...
// RemoveMe: ???
		#region method ContainsCmdLine

	/*	/// <summary>
		/// Checks if there is command line in Socket buffer.
		/// </summary>
		/// <param name="socket"></param>
		/// <returns></returns>
		public static bool ContainsCmdLine(BufferedSocket socket)
		{
			byte[] data = socket.Buffer;
			for(int i=0;i<data.Length;i++){
				if(data[i] == 13){
					if(data.Length > i && data[i + 1] == 10){
						return true;
					}					
				}
			}
